...
void add(LweSample *sum, const LweSample *x, const LweSample *y, const LweSample *c, const int64_t nb_bits, const TFheGateBootstrappingCloudKeySet *keyset)
...
void addmp(LweSample *sum, const LweSample *x, const LweSample *y, const LweSample *c, const int64_t nb_bits, const TFheGateBootstrappingCloudKeySet *keyset)
...
int main() {
    ...
    LweSample* ciphertext3 = new_gate_bootstrapping_ciphertext_array(64, params);



    // reads the 2x64 ciphertexts from the cloud file
    ...
    add(result, ciphertext1, ciphertext2, ciphertext3, 64, bk);
    ...
    FILE* ciphertext1_data = fopen("ciphertext1.data","wb");
    for (int i=0; i<64; i++)
        export_gate_bootstrapping_ciphertext_toFile(ciphertext1_data, &ciphertext1[i], params);
    fclose(ciphertext1_data);

    FILE* ciphertext2_data = fopen("ciphertext2.data","wb");
    for (int i=0; i<64; i++)
        export_gate_bootstrapping_ciphertext_toFile(ciphertext2_data, &ciphertext2[i], params);
    ...
    delete_gate_bootstrapping_ciphertext_array(64, ciphertext2);

    delete_gate_bootstrapping_ciphertext_array(64, ciphertext1);

    delete_gate_bootstrapping_ciphertext_array(64, ciphertext3);
